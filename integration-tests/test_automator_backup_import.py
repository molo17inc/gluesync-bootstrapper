...
class ImportPipelineConfigOnlyTests(unittest.TestCase):
    ...
    def test_gcp_cert_store_sends_credentials_during_import(self):
        """Hub 2.2.11.1+ requires credentials before cert-store; do not defer creds."""
        import automator_app.corehub as corehub

        gcp_config = {
            "pipelineName": "GcpCertStore",
            "agents": [
                {
                    "agentType": "SOURCE",
                    "agentTag": "mssql",
                ...
                },
                {
                    ...
                    "agentInternalName": "google-bigquery",
                    "hostCredentials": {
                        "connectionName": "bigquery",
                        "host": "gluesync-demo",
                        "certificatePath": "certs/sa.json",
        ...
        captured = []
        add_ids = {"mssql": "mssql-agent", "google-bigquery": "bq-agent"}
        ...
        def fake_fetch(path, method="GET", token=None, body=None, **kwargs):
            ...
            if path == "/pipelines" and method == "GET":
                return []
            if path == "/pipelines" and method == "POST":
                return {"pipelineId": "gcp-pipe", "name": body.get("name")}
            if "/agents/add" in path:
                params = kwargs.get("params") or {}
                name = params.get("agentInternalName") or params.get("agentUserTag")
                return {"agentId": add_ids.get(name, "unknown-agent")}
            if "/config/credentials" in path and method == "PUT":
                return {"ok": True}
            if "/agents/" in path and method == "PUT":
                return {"ok": True}
        ...
        with mock.patch.object(corehub, "fetch_core_hub", side_effect=fake_fetch), \
             mock.patch.object(
                 corehub,
                 "_load_agent_type_catalog",
                 return_value={"mssql": "RDBMS", "google-bigquery": "ANALYTICS"},
             ):
            result = corehub.import_pipeline_config_only(
                token="tok",
                base_url="http://test",
        ...
        self.assertEqual(result["pipelineId"], "gcp-pipe")

        cred_puts = [c for c in captured if c["method"] == "PUT" and "/config/credentials" in c["path"]]
        self.assertTrue(cred_puts, "credentials must be PUT during import, not deferred")
        bq_cred = [c for c in cred_puts if "bq-agent" in c["path"]]
        self.assertTrue(bq_cred, "BQ agent credentials must be uploaded during import")
        host_creds = (bq_cred[0]["body"] or {}).get("hostCredentials") or {}
        self.assertNotIn("certificatePath", host_creds)
        self.assertNotIn("certificateType", host_creds)
        self.assertEqual(host_creds.get("connectionName"), "bigquery")

        bq_agent = gcp_config["agents"][1]
        self.assertEqual(bq_agent.get("_certificate_path"), "certs/sa.json")
        self.assertEqual(bq_agent.get("_certificate_type"), "cert-store")
        self.assertNotIn(
            "_host_credentials",
            bq_agent,
            "must not stash credentials to send after certificate",
        )
