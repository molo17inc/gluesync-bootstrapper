...
const normalizeRelativeUrl = (url) => {
  ...
  return BASE_PATH ? `${BASE_PATH}${leading}` : leading;
};

const originalFetch = window.fetch.bind(window);
window.fetch = (input, init) => {
  if (typeof input === "string") {
    return originalFetch(normalizeRelativeUrl(input), init);
  }
  if (input instanceof Request) {
    const rewritten = normalizeRelativeUrl(input.url);
...
const api = {
  ...
  async exportPipelineFull(pipelineId) {
    const response = await fetch(
      `/api/export/pipeline/${encodeURIComponent(pipelineId)}/full`,
    );
    if (!response.ok) {
      throw new Error(
        `Full backup export failed: ${response.status} ${response.statusText}`,
      );
    }
    ...
    const filename =
      response.headers
        ...
        ?.split("filename=")[1]
        ?.replace(/"/g, "") || `pipeline_${pipelineId}_backup.zip`;
  ...
  async getCurrentRun(includeLogs = false) {
    const res = await fetch(
      `/api/run/current${includeLogs ? "?include_logs=true" : ""}`,
    );
  ...
  async bulkListSchemas(pipelineId) {
    const res = await fetch(
      `/api/bulk/schemas?pipelineId=${encodeURIComponent(pipelineId)}`,
    );
  ...
  async bulkListTables(pipelineId, schema) {
    const res = await fetch(
      `/api/bulk/tables?pipelineId=${encodeURIComponent(pipelineId)}&schema=${encodeURIComponent(schema)}`,
    );
  ...
  async bulkTemplate(payload) {
    ...
    const filename =
      res.headers
        .get("Content-Disposition")
        ?.split("filename=")[1]
        ?.replace(/"/g, "") || "template.yaml";
    return { blob, filename };
  },
  async exportPipeline(pipelineId) {
    const response = await fetch(
      `/api/export/pipeline/${encodeURIComponent(pipelineId)}`,
    );
    if (!response.ok) {
      throw new Error(
  ...
  async exportAllPipelines() {
    ...
    if (!response.ok) {
      throw new Error(
        `Export all failed: ${response.status} ${response.statusText}`,
      );
    }
    const blob = await response.blob();
    const filename =
      response.headers
        .get("Content-Disposition")
        ?.split("filename=")[1]
        ?.replace(/"/g, "") || "pipeline_backups.zip";
    return { blob, filename };
  },
  ...
  async duplicatePipeline(payload, options = {}) {
    ...
    const res = await fetch(
      `/api/duplicate/pipeline/${encodeURIComponent(payload.pipelineId)}`,
      requestInit,
    );
    if (!res.ok) {
      const detail = await res.json().catch(() => ({}));
...
const ui = (() => {
  ...
  const exportPipelineFullBtn = document.getElementById(
    "export-pipeline-full-btn",
  );
  ...
  function makeLogNode(text) {
    if (
      logTemplate &&
      logTemplate.content &&
      logTemplate.content.firstElementChild
    ) {
...
const corehubUI = (() => {
  ...
  const runningContainerTotalEl = document.getElementById(
    "corehub-running-container-count",
  );
  ...
  const getChronosJobStats = (service = {}) => {
    const enabled = Number.isFinite(service.enabledJobs)
      ? service.enabledJobs
      : 0;
    let total = Number.isFinite(service.totalJobs)
      ? service.totalJobs
      : enabled;
    if (total < enabled) {
      total = enabled;
  ...
  const renderPipelines = (pipelines = []) => {
    ...
    pipelines.forEach((pipeline) => {
      ...
      const pipelineId =
        pipeline.pipelineId || pipeline.id || pipeline.pipeline_id || "";
      const pipelineName = pipeline.name || pipelineId || "Unnamed pipeline";
      title.textContent = pipelineId
  ...
  return {
    updateEnvironment(state) {
      ...
      setText(
        verifyEl,
        formatToggle(state.skipVerify, ["Enforced", "Skipped"]),
      );
...
function initTabs() {
  ...
  const hideTab = (tabName, hidden) => {
    ...
    if (hidden && button.classList.contains("active")) {
      const fallback = tabButtons.find(
...
const stateManager = {
  ...
  updateFromState(data) {
    ...
    document.getElementById("enable-scheduling").checked =
...
async function initialize() {
  try {
    ...
    if (snapshot.tokenPresent) {
      ...
      await Promise.all([loadPipelines(), refreshCorehubOverview()]);
    } else {
...
async function checkAutomatorVersion() {
  ...
  try {
    const res = await fetch(
      "https://api.backoffice.molo17.com/agent/automator/version",
    );
    if (!res.ok) {
    ...
    const data = await res.json();
    const latestGa =
      data.latestVersionGA || data.latestVersionGa || data.latestversionGA;
    if (!latestGa) {
      return;
    }
    ...
    try {
      ...
      const changelog =
        clJson && typeof clJson.changelog === "string" ? clJson.changelog : "";

      if (changelog) {
        console.info(`Automator ${latestGa} changelog:\n${changelog}`);
        const container =
          changelogContainer || document.getElementById("version-changelog");
        if (container) {
...
function updateImportButtonsVisibility() {
  ...
  if (importConfigBtnEl) {
    const hasConfigFile = !!(
      importConfigInput &&
      importConfigInput.files &&
      importConfigInput.files[0]
    );
  ...
  const hasZipFile = !!(
    importAllInput &&
    importAllInput.files &&
    importAllInput.files[0]
  );
...
function bindEvents() {
  ...
  function updateBulkSelectionSummary() {
    ...
    if (!total) {
      bulkTablesSummary.textContent =
        bulkSchemaSelect && bulkSchemaSelect.value
  ...
  if (bulkLoadSchemasBtn && bulkSchemaSelect) {
    bulkLoadSchemasBtn.disabled = !(
      bulkPipelineSelect && bulkPipelineSelect.value
    );
    bulkLoadSchemasBtn.addEventListener("click", async () => {
      ...
      logActivity(
        "Bulk",
        `Loading source schemas for pipeline ${bulkPipelineSelect.value}`,
      );
      ...
      try {
        ...
        if (!schemas.length) {
          ui.setBulkMessage(
            "No source schemas found for this pipeline",
            "error",
          );
          logActivity(
            "Bulk",
            `No schemas returned for pipeline ${bulkPipelineSelect.value}`,
          );
        } else {
          ui.setBulkMessage(
            `Loaded ${schemas.length} schema(s). Select a schema to load tables.`,
            "success",
          );
          logActivity(
            "Bulk",
            `Loaded ${schemas.length} schema(s) for pipeline ${bulkPipelineSelect.value}`,
          );
        }
      } catch (err) {
  ...
  if (bulkDownloadYamlBtn) {
    bulkDownloadYamlBtn.addEventListener("click", async () => {
      ...
      const selectedTables = bulkTablesState
        .filter((t) => t.selected)
        .map((t) => t.name);
      ...
      logActivity(
        "Bulk",
        `Generating YAML template for ${selectedTables.length} table(s) in schema ${bulkSchemaSelect.value}`,
      );
  ...
  if (bulkSchemaSelect) {
    bulkSchemaSelect.addEventListener("change", async () => {
      ...
      try {
        const res = await api.bulkListTables(
          bulkPipelineSelect.value,
          bulkSchemaSelect.value,
        );
        ...
        if (!tables.length) {
          ...
          logActivity(
            "Bulk",
            `Loaded ${tables.length} table(s) for schema ${bulkSchemaSelect.value}`,
          );
        }
      } catch (err) {
        ui.setBulkMessage(err.message, "error");
  ...
  if (bulkToggleAllBtn) {
    bulkToggleAllBtn.addEventListener("click", () => {
      ...
      if (bulkTablesList) {
        const inputs = bulkTablesList.querySelectorAll(
          'input[type="checkbox"]',
        );
  ...
  if (bulkCreateBtn) {
    bulkCreateBtn.addEventListener("click", async () => {
      ...
      const targetSchema =
        bulkTargetSchemaInput && bulkTargetSchemaInput.value
          ? bulkTargetSchemaInput.value
          : sourceSchema;

      const chunkSizeInput = document.getElementById("chunk-size");
      const skipErrorsInput = document.getElementById("skip-errors");
      const enableSchedulingInput =
        document.getElementById("enable-scheduling");
      ...
      const payload = {
        ...
        enableScheduling: !!(
          enableSchedulingInput && enableSchedulingInput.checked
        ),
      ...
      logActivity(
        "Bulk",
        `Starting bulk entity creation for ${selectedTables.length} table(s) from schema ${sourceSchema}`,
      );
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        if (attempt > 1) {
          ...
          ui.setAuthMessage(
            `Retrying authentication (attempt ${attempt}/${maxRetries})…`,
          );
          logActivity(
            "Auth",
            `Retrying authentication (attempt ${attempt}/${maxRetries})…`,
        ...
        logActivity(
          "Auth",
          `Network error on attempt ${attempt}, will retry...`,
        );
      }
    }
  ...
  ui.logoutBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (exportPipelineSelect) {
        exportPipelineSelect.innerHTML =
      ...
      if (configPipelineSelect && configPipelineSelect.tagName === "SELECT") {
        configPipelineSelect.innerHTML =
          '<option value="">Select a pipeline…</option>';
      }
      const duplicatePipelineSelect = document.getElementById(
        "duplicate-pipeline-id",
      );
      if (
        duplicatePipelineSelect &&
        duplicatePipelineSelect.tagName === "SELECT"
      ) {
        duplicatePipelineSelect.innerHTML =
          '<option value="">Select a pipeline to duplicate…</option>';
      }
  ...
  configForm.addEventListener("submit", async (event) => {
    ...
    if (!stateManager.yamlFileId) {
      ui.setConfigMessage(
        "Please upload a YAML configuration file first",
        "error",
      );
      return;
    }

    const customSchemasEnabled = customSchemasCheckbox
      ? customSchemasCheckbox.checked
      : false;
    ...
    logActivity(
      "Config",
      `Starting entity creation for pipeline ${payload.pipelineId || "(none)"}…`,
    );
  ...
  if (exportForm && exportPipelineSelect) {
    exportForm.addEventListener("submit", async (event) => {
      ...
      if (!pipelineId) {
        ui.setExportMessage(
          "Please select a pipeline to export metadata for",
          "error",
        );
        return;
      }
  ...
  if (ui.exportPipelineFullBtn && exportPipelineSelect) {
    ui.exportPipelineFullBtn.addEventListener("click", async () => {
      ...
      if (!pipelineId) {
        ui.setExportMessage(
          "Please select a pipeline to export a full backup for",
          "error",
        );
        return;
      }
      ...
      logActivity(
        "Export",
        `Exporting full backup for pipeline ${pipelineId}…`,
      );
      try {
  ...
  if (importConfigBtnEl && importConfigInput) {
    importConfigBtnEl.addEventListener("click", async () => {
      ...
      try {
        ...
        ui.setConfigMessage(
          result.message || "Config imported successfully",
          "success",
        );
  ...
  if (importAllBtnEl && importAllInput) {
    importAllBtnEl.addEventListener("click", async () => {
      ...
      const autoDeployAgents = autoDeployCheckbox
        ? autoDeployCheckbox.checked
        : true;
      ...
      try {
        ...
        if (result.success === false) {
          ...
          ui.setConfigMessage(
            result.message || "Backup imported successfully",
            "success",
          );
          logActivity(
            "Config",
            result.message || "Full backup import completed",
          );
        }
      } catch (err) {
        ui.setConfigMessage(err.message, "error");
  ...
  if (validateAllBtnEl && importAllInput) {
    validateAllBtnEl.addEventListener("click", async () => {
      ...
      try {
        ...
        logActivity(
          "Config",
          ok ? "Backup validation OK" : "Backup validation failed",
        );
      } catch (err) {
        ui.setConfigMessage(err.message, "error");
  ...
  function setDuplicateBusy(isBusy, label) {
    ...
    const defaultLabel =
      duplicateBtn.dataset.defaultLabel || "Duplicate Pipeline";
    duplicateBtn.textContent = isBusy
      ? label || "Stop duplication"
      : defaultLabel;
    if (isBusy) {
  ...
  if (duplicateForm) {
    duplicateForm.addEventListener("submit", async (event) => {
      ...
      if (duplicateRequestController) {
        try {
          ...
          ui.setDuplicateMessage(
            response.message || "Cancelling duplicate request…",
            "warning",
          );
          logActivity(
            "duplicate",
            response.message || "Cancellation requested by user",
          );
        } catch (err) {
          ui.setDuplicateMessage(
          ...
          logActivity(
            "duplicate",
            `Failed to request cancellation: ${err.message}`,
          );
        } finally {
      ...
      if (!pipelineId) {
        ui.setDuplicateMessage(
          "Please select a pipeline to duplicate",
          "error",
        );
        return;
      }
      logActivity(
        "duplicate",
        `Starting duplication for pipeline ${pipelineId}`,
      );

      const sourcePasswordInput = document.getElementById(
        "duplicate-source-password",
      );
      const targetPasswordInput = document.getElementById(
        "duplicate-target-password",
      );
      const sourcePassword = sourcePasswordInput
        ? sourcePasswordInput.value.trim()
        : "";
      const targetPassword = targetPasswordInput
        ? targetPasswordInput.value.trim()
        : "";

      if (!sourcePassword || !targetPassword) {
        ui.setDuplicateMessage(
          "Both source and target agent passwords are required.",
          "error",
        );
        return;
      }

      const customizeAgentsEnabled = !!(
        document.getElementById("customize-agents") &&
        document.getElementById("customize-agents").checked
      );
      ...
      if (!sourceAgentTagValue || !targetAgentTagValue) {
        ui.setDuplicateMessage(
          "Current agent tags are unavailable. Please fetch pipeline details or enable customization.",
          "error",
        );
        return;
      }
      ...
      const newPipelineNameInput =
        document.getElementById("duplicate-new-name");
      const newPipelineNameValue = newPipelineNameInput
        ? newPipelineNameInput.value.trim()
        : "";
      const cloneEntitiesToggle = document.getElementById(
        "duplicate-clone-entities",
      );
      const cloneEntitiesValue = cloneEntitiesToggle
        ? cloneEntitiesToggle.checked
        : true;
      ...
      const payload = {
        ...
        conductorUrl:
          document.getElementById("duplicate-conductor-url").value || undefined,
      };
      if (payload.conductorUrl) {
        logActivity(
          "duplicate",
          `Using custom Conductor URL: ${payload.conductorUrl}`,
        );
      } else {
      ...
      const sourceHostValue = sourceHostInput
        ? sourceHostInput.value.trim()
        : "";
      const targetHostValue = targetHostInput
        ? targetHostInput.value.trim()
        : "";
      if (sourceHostValue) {
        payload.sourceHost = sourceHostValue;
        logActivity(
          "duplicate",
          `Overriding source host to ${sourceHostValue}`,
        );
      }
      if (targetHostValue) {
        ...
        logActivity(
          "duplicate",
          `Overriding target host to ${targetHostValue}`,
        );
      }

      const customizeSchemasToggle =
        document.getElementById("customize-schemas");
      if (customizeSchemasToggle && customizeSchemasToggle.checked) {
        const sourceSchemaInput = document.getElementById(
          "duplicate-source-schema",
        );
        const targetSchemaInput = document.getElementById(
          "duplicate-target-schema",
        );
        const sourceSchemaValue = sourceSchemaInput
          ? sourceSchemaInput.value.trim()
          : "";
        const targetSchemaValue = targetSchemaInput
      ...
      ui.setDuplicateMessage(
        'Duplicating pipeline… exporting configuration and checking agent availability. Click "Stop" to cancel.',
        "info",
      );
      logActivity("duplicate", "Payload sent to backend. Awaiting response…");
      try {
        const result = await api.duplicatePipeline(payload, {
          signal: duplicateRequestController.signal,
        });
        const duplicatedName =
          result.newPipelineName || result.pipelineName || "(unnamed pipeline)";
        const duplicatedId =
          result.newPipelineId || result.pipelineId || "(unknown id)";
        ui.setDuplicateMessage(
          `Pipeline duplicated successfully: ${duplicatedName} (${duplicatedId})`,
          "success",
        );
        logActivity(
          "duplicate",
          `Duplicate succeeded: ${duplicatedName} (${duplicatedId})`,
        );
        ...
        if (err.name === "AbortError") {
          ui.setDuplicateMessage(
            "Duplicate request cancelled by user",
            "warning",
          );
  ...
  if (duplicatePipelineSelect) {
    const setCurrentTags = (sourceTag = "", targetTag = "") => {
      const currentSourceTagInput = document.getElementById(
        "duplicate-current-source-tag",
      );
      const currentTargetTagInput = document.getElementById(
        "duplicate-current-target-tag",
      );
    ...
    duplicatePipelineSelect.addEventListener("change", async () => {
      ...
      try {
        const response = await fetch(
          `/api/pipeline/${encodeURIComponent(pipelineId)}/agents`,
        );
        ...
        console.error(
          "Failed to fetch pipeline agents for auto-population:",
          err,
        );
        ui.setDuplicateMessage(
          err.message || "Failed to fetch pipeline agents",
          "error",
        );
      }
    });
  ...
  const agentCustomizationFields = document.getElementById(
    "agent-customization-fields",
  );
  const customizeConductorCheckbox = document.getElementById(
    "customize-conductor",
  );
  ...
  const schemaOverrideFields = document.getElementById(
  ...
  if (customizeAgentsCheckbox) {
    ...
    customizeAgentsCheckbox.addEventListener(
      "change",
      updateAgentCustomizationVisibility,
    );
  }
  if (customizeConductorCheckbox) {
    ...
    customizeConductorCheckbox.addEventListener(
      "change",
      updateConductorVisibility,
    );
  }
  if (customizeSchemasCheckbox) {
    ...
    customizeSchemasCheckbox.addEventListener(
      "change",
      updateSchemaOverrideVisibility,
    );
  }
...
async function loadPipelines() {
  ...
  try {
    ...
    const populateSelect = (selectEl) => {
      ...
      const placeholder =
        selectEl.id === "duplicate-pipeline-id"
          ? "Select a pipeline to duplicate…"
          : "Select a pipeline…";
      ...
      pipelines.forEach((p) => {
        ...
        const displayId =
          pipelineId.length > 12 ? pipelineId.slice(0, 8) + "…" : pipelineId;
