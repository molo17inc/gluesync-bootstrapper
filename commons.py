...
def create_group(token, pipeline_id, group_name):
    ...
    Returns the group ID or '_default' if group creation is not supported.
    """
    ...
    # Check if groups endpoint is available
    try:
        # Try to access the groups endpoint
        groups = fetch_core_hub(
            f"/pipelines/{pipeline_id}/config",
            method='GET',
            token=token
        )
        logger.debug(f"Pipeline config: {json.dumps(groups, indent=2) if groups else 'No config'}")
        
        # If we get here but no groups endpoint, log and return default
        logger.warning(f"Groups endpoint not found in pipeline config. Group '{group_name}' will use default group.")
        return "_default"
        
    except Exception as e:
        logger.warning(f"Failed to check groups endpoint: {str(e)}")
