...
The exporter scripts (`export_template_from_corehub.py`, `export_all_pipelines.py`) populate this section from CoreHub when generating backup YAML files. Instance-level settings are exported separately as `global-config.yaml`, which is a direct dump of CoreHub's `/global-config` payload.
