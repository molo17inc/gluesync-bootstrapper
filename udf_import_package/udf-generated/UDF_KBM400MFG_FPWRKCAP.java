 ...
 * Entity: KBM400MFG.FPWRKCAP (DbMoto ReplicationID: 280)
 *
 * Decoded DbMoto field-mapping expressions:
 *   MGEFDATE <- LowDate(MakeDateTime([MGEFMO],[MGEFDA],[MGEFYR],[MGEFCT]))
 *
 ...
 */
public class UDF_KBM400MFG_FPWRKCAP {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: WCCO, WCNO, MGCAP, MGMAX, MGSHFT, SFTLEN, EFFFAC, MCHPWC, PPLPWC, MGEFYR, MGEFMO, MGEFDA, MGEFCT, WCCPQT
        // Target columns: WCCO, WCNO, MGCAP, MGMAX, MGSHFT, SFTLEN, EFFFAC, MCHPWC, PPLPWC, MGEFDATE, WCCPQT

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("MGEFDATE", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("MGEFDATE", lowDate(makeDateTime(toBigDecimal(newValues.get("MGEFMO")), toBigDecimal(newValues.get("MGEFDA")), toBigDecimal(newValues.get("MGEFYR")), toBigDecimal(newValues.get("MGEFCT")))));
        } catch (Exception e) {
