 ...
 * Entity: KBMEFCOADD.REPUSRF (DbMoto ReplicationID: 709)
 *
 * Decoded DbMoto field-mapping expressions:
 *   REPUSRLUDATETIME <- MakeDateTime([REPUSRLUDT],[REPUSRLUTM])
 *   REPUSRCTDATETIME <- MakeDateTime([REPUSRCTDT],[REPUSRCTTM])
 *   REPUSRUPDATETIME <- MakeDateTime([REPUSRUPDT],[REPUSRUPTM])
 *
 ...
 */
public class UDF_KBMEFCOADD_REPUSRF {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: REPUSRTYP, REPUSRKEY, REPLOGID, REPUSRSEQ, REPUSRPW, REPUSRSW, REPUSRDESC, REPUSRLUDT, REPUSRLUTM, REPUSRUSES, REPUSRSTAT, REPUSRCOMM, REPUSRSCOM, REPUSRCTDT, REPUSRCTTM, REPUSRCTID, REPUSRUPDT, REPUSRUPTM, REPUSRUPID
        // Target columns: REPUSRTYP, REPUSRKEY, REPLOGID, REPUSRSEQ, REPUSRPW, REPUSRSW, REPUSRDESC, REPUSRLUDATETIME, REPUSRUSES, REPUSRSTAT, REPUSRCOMM, REPUSRSCOM, REPUSRCTDATETIME, REPUSRCTID, REPUSRUPDATETIME, REPUSRUPID

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("REPUSRLUDATETIME", null);
            modified_values.put("REPUSRCTDATETIME", null);
            modified_values.put("REPUSRUPDATETIME", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("REPUSRLUDATETIME", makeDateTime(toLocalDate(newValues.get("REPUSRLUDT")), toLocalTime(newValues.get("REPUSRLUTM"))));
                modified_values.put("REPUSRCTDATETIME", makeDateTime(toLocalDate(newValues.get("REPUSRCTDT")), toLocalTime(newValues.get("REPUSRCTTM"))));
                modified_values.put("REPUSRUPDATETIME", makeDateTime(toLocalDate(newValues.get("REPUSRUPDT")), toLocalTime(newValues.get("REPUSRUPTM"))));
        } catch (Exception e) {
