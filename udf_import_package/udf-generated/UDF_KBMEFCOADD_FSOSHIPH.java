 ...
 * Entity: KBMEFCOADD.FSOSHIPH (DbMoto ReplicationID: 331)
 *
 * Decoded DbMoto field-mapping expressions:
 *   SOSDATE <- LowDate(MakeDateTime([SOSDATE],[SOSTIME]))
 *   SOSDEPDT <- MakeDateTime([SOSDEPDT])
 *   SOSDDT <- MakeDateTime([SOSDDT],[SOSDTM])
 *
 ...
 */
public class UDF_KBMEFCOADD_FSOSHIPH {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: SOSCO, SOSSO, SOSDASH, SOSDATE, SOSTIME, SOSCAR, SOSTRL, SOSCNM, SOSDEPDT, SOSDDT, SOSDTM, SOSDSCU, SOSDSCR, SOSLL, SOSWTS, SOSBIL
        // Target columns: SOSCO, SOSSO, SOSDASH, SOSDATE, SOSCAR, SOSTRL, SOSCNM, SOSDEPDT, SOSDDT, SOSDSCU, SOSDSCR, SOSLL, SOSWTS, SOSBIL

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("SOSDATE", null);
            modified_values.put("SOSDEPDT", null);
            modified_values.put("SOSDDT", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("SOSDATE", lowDate(makeDateTime(toLocalDate(newValues.get("SOSDATE")), toLocalTime(newValues.get("SOSTIME")))));
                modified_values.put("SOSDEPDT", makeDateTime(toLocalDate(newValues.get("SOSDEPDT"))));
                modified_values.put("SOSDDT", makeDateTime(toLocalDate(newValues.get("SOSDDT")), toLocalTime(newValues.get("SOSDTM"))));
        } catch (Exception e) {
