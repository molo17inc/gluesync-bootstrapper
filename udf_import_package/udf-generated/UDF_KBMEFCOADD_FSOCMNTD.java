 ...
 * Entity: KBMEFCOADD.FSOCMNTD (DbMoto ReplicationID: 328)
 *
 * Decoded DbMoto field-mapping expressions:
 *   SODCDAT <- LowDate(MakeDateTime([SODCDAT],[SODCTIM]))
 *
 ...
 */
public class UDF_KBMEFCOADD_FSOCMNTD {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: SODDCO, SODNUM, SODLINE, SODOPT, SODCDAT, SODCTIM, SODSEQ, SODCMNT, SODPART, SODDASH
        // Target columns: SODDCO, SODNUM, SODLINE, SODOPT, SODCDAT, SODSEQ, SODCMNT, SODPART, SODDASH

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("SODCDAT", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("SODCDAT", lowDate(makeDateTime(toLocalDate(newValues.get("SODCDAT")), toLocalTime(newValues.get("SODCTIM")))));
        } catch (Exception e) {
