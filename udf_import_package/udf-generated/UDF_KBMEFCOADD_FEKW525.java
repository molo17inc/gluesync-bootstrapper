 ...
 * Entity: KBMEFCOADD.FEKW525 (DbMoto ReplicationID: 300)
 *
 * Decoded DbMoto field-mapping expressions:
 *   SCCRTDATE <- MakeDateTime([SCCRTD],[SCCRTT])
 *   SCUPDATE <- MakeDateTime([SCUPDD], [SCUPDT])
 *
 ...
 */
public class UDF_KBMEFCOADD_FEKW525 {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: PSCO, PSPMRN, PSCMRN, PSSEQ, PSCML1, PSCML2, SCCRTD, SCUPDD, SCCRTT, SCUPDT, SCCRTU, SCUPDU, SCPGMN
        // Target columns: PSCO, PSPMRN, PSCMRN, PSSEQ, PSCML1, PSCML2, SCCRTDATE, SCUPDATE, SCCRTU, SCUPDU, SCPGMN

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("SCCRTDATE", null);
            modified_values.put("SCUPDATE", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("SCCRTDATE", makeDateTime(toLocalDate(newValues.get("SCCRTD")), toLocalTime(newValues.get("SCCRTT"))));
                modified_values.put("SCUPDATE", makeDateTime(toLocalDate(newValues.get("SCUPDD")), toLocalTime(newValues.get("SCUPDT"))));
        } catch (Exception e) {
