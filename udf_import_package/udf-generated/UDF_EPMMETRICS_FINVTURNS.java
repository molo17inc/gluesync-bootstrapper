 ...
 * Entity: EPMMETRICS.FINVTURNS (DbMoto ReplicationID: 195)
 *
 * Decoded DbMoto field-mapping expressions:
 *   DATADATE <- LowDate(MakeDateTime([DATADATE]))
 *
 ...
 */
public class UDF_EPMMETRICS_FINVTURNS {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: TSCO, YEAR, PERIOD, WEEK, DATADATE, TSPN, IMCLS, IMDSC, ONHAND, ISSUED, IMLCOS, INV$, ISS$, QIS30DA, COST30D, AVEUSE, AVECOS, AVE$, COMMENT
        // Target columns: TSCO, YEAR, PERIOD, WEEK, DATADATE, TSPN, IMCLS, IMDSC, ONHAND, ISSUED, IMLCOS, INV$, ISS$, QIS30DA, COST30D, AVEUSE, AVECOS, AVE$, COMMENT

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("DATADATE", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("DATADATE", lowDate(makeDateTime(toLocalDate(newValues.get("DATADATE")))));
        } catch (Exception e) {
