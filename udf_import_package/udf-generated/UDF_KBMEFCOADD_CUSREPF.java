 ...
 * Entity: KBMEFCOADD.CUSREPF (DbMoto ReplicationID: 702)
 *
 * Decoded DbMoto field-mapping expressions:
 *   CRCTDATETIME <- MakeDateTime([CRCTDT],[CRCTTM])
 *   CRUPDATETIME <- MakeDateTime([CRUPDT],[CRUPTM])
 *
 ...
 */
public class UDF_KBMEFCOADD_CUSREPF {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: CRGROUP, CRCUSREP, CRPHNEEXT, CRGRPCODE, CRCTDT, CRCTTM, CRCTID, CRUPDT, CRUPTM, CRUPID, CRRMPN
        // Target columns: CRGROUP, CRCUSREP, CRPHNEEXT, CRGRPCODE, CRCTDATETIME, CRCTID, CRUPDATETIME, CRUPID, CRRMPN

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("CRCTDATETIME", null);
            modified_values.put("CRUPDATETIME", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("CRCTDATETIME", makeDateTime(toLocalDate(newValues.get("CRCTDT")), toLocalTime(newValues.get("CRCTTM"))));
                modified_values.put("CRUPDATETIME", makeDateTime(toLocalDate(newValues.get("CRUPDT")), toLocalTime(newValues.get("CRUPTM"))));
        } catch (Exception e) {
