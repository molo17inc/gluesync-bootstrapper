 ...
 * Entity: KBMEFCOADD.FEKMLODD (DbMoto ReplicationID: 339)
 *
 * Decoded DbMoto field-mapping expressions:
 *   EDATE <- MakeDateTime([EDATE])
 *   LDCRTDT <- MakeDateTime([LDCRTD],[LDCRTT])
 *   LDUPDDT <- MakeDateTime([LDUPDD],[LDUPDT])
 *
 ...
 */
public class UDF_KBMEFCOADD_FEKMLODD {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: CHCO, LOAD, CHONO, WOWONO, PSSEQ, DIEPAR, PIECES, SQFTPC, SQFTEX, EDATE, LDCRTD, LDUPDD, LDCRTT, LDUPDT, LDCRTU, LDUPDU, LDPGMN, STATUS, ORIGQTY
        // Target columns: CHCO, LOAD, CHONO, WOWONO, PSSEQ, DIEPAR, PIECES, SQFTPC, SQFTEX, EDATE, LDCRTDT, LDUPDDT, LDCRTU, LDUPDU, LDPGMN, RECORDID

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("EDATE", null);
            modified_values.put("LDCRTDT", null);
            modified_values.put("LDUPDDT", null);
            modified_values.put("RECORDID", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("EDATE", makeDateTime(toLocalDate(newValues.get("EDATE"))));
                modified_values.put("LDCRTDT", makeDateTime(toLocalDate(newValues.get("LDCRTD")), toLocalTime(newValues.get("LDCRTT"))));
                modified_values.put("LDUPDDT", makeDateTime(toLocalDate(newValues.get("LDUPDD")), toLocalTime(newValues.get("LDUPDT"))));
