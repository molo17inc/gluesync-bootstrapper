 ...
 * Entity: ESPRODLIB.FSPPDSC (DbMoto ReplicationID: 696)
 *
 * Decoded DbMoto field-mapping expressions:
 *   CACRTDATETIME <- MakeDateTime([CACRTD],[CACRTT])
 *   CAUPDDATETIME <- MakeDateTime([CAUPDD],[CAUPDT])
 *
 ...
 */
public class UDF_ESPRODLIB_FSPPDSC {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: PRTIDT, PRTD, CACO, CACRTD, CAUPDD, CACRTT, CAUPDT, CACRTU, CAUPDU, CAPGMN
        // Target columns: PRTIDT, PRTD, CACO, CACRTDATETIME, CAUPDDATETIME, CACRTU, CAUPDU, CAPGMN

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("CACRTDATETIME", null);
            modified_values.put("CAUPDDATETIME", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("CACRTDATETIME", makeDateTime(toLocalDate(newValues.get("CACRTD")), toLocalTime(newValues.get("CACRTT"))));
                modified_values.put("CAUPDDATETIME", makeDateTime(toLocalDate(newValues.get("CAUPDD")), toLocalTime(newValues.get("CAUPDT"))));
        } catch (Exception e) {
