 ...
 * Entity: INTRANET.PT_TIMELOG (DbMoto ReplicationID: 231)
 *
 * Decoded DbMoto field-mapping expressions:
 *   LOGGED_DATE <- MakeDateTime([LOGGED_DATE])
 *
 ...
 */
public class UDF_INTRANET_PT_TIMELOG {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: TIMELOG_ID, EMPLOYEE_ID, TASK_ID, LOGGED_HOURS, LOGGED_DATE, BRKDWN_ID, COMMENTS, BEGIN_TIME, END_TIME
        // Target columns: TIMELOG_ID, EMPLOYEE_ID, TASK_ID, LOGGED_HOURS, LOGGED_DATE, BRKDWN_ID, COMMENTS, BEGIN_TIME, END_TIME

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("LOGGED_DATE", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("LOGGED_DATE", makeDateTime(toLocalDate(newValues.get("LOGGED_DATE"))));
        } catch (Exception e) {
