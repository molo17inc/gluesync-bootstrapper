 ...
 * Entity: KBMEFCOADD.FSENGHRS (DbMoto ReplicationID: 326)
 *
 * Decoded DbMoto field-mapping expressions:
 *   SEECrtDdtTm <- MakeDateTime([SEECRTDT],[SEECRTTM])
 *   SEEUpdDtTm <- MakeDateTime([SEEUPDDT],[SEEUPDTM])
 *
 ...
 */
public class UDF_KBMEFCOADD_FSENGHRS {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: SECO, SEUSERID, SENAME, SEYEAR, SEWEEK, SEEMPHRS, SEAVLHRS, SEOVRTIM, SEMISCHRS, SEHRSCOMPL, SEECUSER, SEECPGMN, SEECRTDT, SEECRTTM, SEEUUSER, SEEUPGMN, SEEUPDDT, SEEUPDTM
        // Target columns: SECo, SEUserId, SEName, SEYear, SEWeek, SEEmpHrs, SEAvlHrs, SEOvrTim, SEMiscHrs, SEHrsCompl, SEECUser, SEECPgmn, SEECrtDdtTm, SEEUuser, SEEUPgmn, SEEUpdDtTm

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("SEECrtDdtTm", null);
            modified_values.put("SEEUpdDtTm", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("SEECrtDdtTm", makeDateTime(toLocalDate(newValues.get("SEECRTDT")), toLocalTime(newValues.get("SEECRTTM"))));
                modified_values.put("SEEUpdDtTm", makeDateTime(toLocalDate(newValues.get("SEEUPDDT")), toLocalTime(newValues.get("SEEUPDTM"))));
        } catch (Exception e) {
