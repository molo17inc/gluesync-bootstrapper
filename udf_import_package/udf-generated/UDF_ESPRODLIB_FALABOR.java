 ...
 * Entity: ESPRODLIB.FALABOR (DbMoto ReplicationID: 1068)
 *
 * Decoded DbMoto field-mapping expressions:
 *   LFUPDDT <- MakeDateTime([LFUPDD], [LFUPDT])
 *   LFCRTDT <- MakeDateTime([LFCRTD], [LFCRTT])
 *   ALDATET <- MakeDateTime([ALDATE], [ALTIME])
 *
...
public class UDF_ESPRODLIB_FALABOR {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        ...
        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("LFUPDDT", null);
            modified_values.put("LFCRTDT", null);
            modified_values.put("ALDATET", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("LFUPDDT", makeDateTime(toLocalDate(newValues.get("LFUPDD")), toLocalTime(newValues.get("LFUPDT"))));
                modified_values.put("LFCRTDT", makeDateTime(toLocalDate(newValues.get("LFCRTD")), toLocalTime(newValues.get("LFCRTT"))));
                modified_values.put("ALDATET", makeDateTime(toLocalDate(newValues.get("ALDATE")), toBigDecimal(newValues.get("ALTIME"))));
        } catch (Exception e) {
