 ...
 * Entity: ESPRODLIB.FEMAPMST (DbMoto ReplicationID: 204)
 *
 * Decoded DbMoto field-mapping expressions:
 *   MCCRTDATE <- MakeDateTime([MCCRTD],[MCCRTT])
 *   MCUPDDATE <- MakeDateTime([MCUPDD],[MCUPDT])
 *
 ...
 */
public class UDF_ESPRODLIB_FEMAPMST {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: MCCO, MCCODE, MCSRTC, MCDES1, MCDESC, MCCRTD, MCUPDD, MCCRTT, MCUPDT, MCCRTU, MCUPDU, MCPGMN
        // Target columns: MCCO, MCCODE, MCSRTC, MCDES1, MCDESC, MCCRTDATE, MCUPDDATE, MCCRTU, MCUPDU, MCPGMN

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("MCCRTDATE", null);
            modified_values.put("MCUPDDATE", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("MCCRTDATE", makeDateTime(toLocalDate(newValues.get("MCCRTD")), toLocalTime(newValues.get("MCCRTT"))));
                modified_values.put("MCUPDDATE", makeDateTime(toLocalDate(newValues.get("MCUPDD")), toLocalTime(newValues.get("MCUPDT"))));
        } catch (Exception e) {
