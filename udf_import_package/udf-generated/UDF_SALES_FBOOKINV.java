 ...
 * Entity: SALES.FBOOKINV (DbMoto ReplicationID: 353)
 *
 * Decoded DbMoto field-mapping expressions:
 *   TIMEPERIOD <- MakeDateTime([TIME_MONTH],[TIME_DAY],[TIME_YEAR],[TIME_CENTURY])
 *
 ...
 */
public class UDF_SALES_FBOOKINV {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: COMPANY, SLSORDNO, SLSLINENO, LINESTS, DASH, CATEGORY, SERIES, PRODUCT, MPSPART, RGNCODE, RGNDESC, SLSMANNO, SLSNAME, CUSTNO, CUSTNAME, TIME_MONTH, TIME_DAY, TIME_YEAR, TIME_CENTURY, BOOKINGAMT, INVOICEAMT
        // Target columns: COMPANY, SLSORDNO, SLSLINENO, LINESTS, DASH, CATEGORY, SERIES, PRODUCT, MPSPART, RGNCODE, RGNDESC, SLSMANNO, SLSNAME, CUSTNO, CUSTNAME, TIMEPERIOD, BOOKINGAMT, INVOICEAMT

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("TIMEPERIOD", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("TIMEPERIOD", makeDateTime(toBigDecimal(newValues.get("TIME_MONTH")), toBigDecimal(newValues.get("TIME_DAY")), toBigDecimal(newValues.get("TIME_YEAR")), toBigDecimal(newValues.get("TIME_CENTURY"))));
        } catch (Exception e) {
