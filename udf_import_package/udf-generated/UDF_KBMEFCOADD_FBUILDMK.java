 ...
 * Entity: KBMEFCOADD.FBUILDMK (DbMoto ReplicationID: 293)
 *
 * Decoded DbMoto field-mapping expressions:
 *   TSDTTRTM <- LowDate(MakeDateTime([TSDTTR],[TSTIME]))
 *
 ...
 */
public class UDF_KBMEFCOADD_FBUILDMK {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: CYEAR, CNMBR, CDASH, MARK, MKQTY, SERIES, OSPN, TSTQTY, TSDTTR, TSTIME, USRPRF, TSQOH, TSUTMS, TSREFN, TSCUSN, TSSLSN, TSLINE, TSSESN, TR, BMSUFFIX
        // Target columns: CYEAR, CNMBR, CDASH, MARK, MKQTY, SERIES, OSPN, TSTQTY, TSDTTRTM, USRPRF, TSQOH, TSUTMS, TSREFN, TSCUSN, TSSLSN, TSLINE, TSSESN, TR

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("TSDTTRTM", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("TSDTTRTM", lowDate(makeDateTime(toLocalDate(newValues.get("TSDTTR")), toBigDecimal(newValues.get("TSTIME")))));
        } catch (Exception e) {
