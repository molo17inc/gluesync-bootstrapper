 ...
 * Entity: ESPRODLIB.MTPFNDES (DbMoto ReplicationID: 217)
 *
 * Decoded DbMoto field-mapping expressions:
 *   DATE <- MakeDateTime([DATE])
 *   CQCRTDT <- MakeDateTime([CQCRTD],[CQCRTT])
 *   CQUPDDT <- MakeDateTime([CQUPDD],[CQUPDT])
 *
...
public class UDF_ESPRODLIB_MTPFNDES {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        ...
        if (operation == MappingFunctionOperation.Delete) {
            ...
            modified_values.put("CQCRTDT", null);
            modified_values.put("CQUPDDT", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("DATE", makeDateTime(toLocalDate(newValues.get("DATE"))));
                modified_values.put("CQCRTDT", makeDateTime(toLocalDate(newValues.get("CQCRTD")), toLocalTime(newValues.get("CQCRTT"))));
                modified_values.put("CQUPDDT", makeDateTime(toLocalDate(newValues.get("CQUPDD")), toLocalTime(newValues.get("CQUPDT"))));
        } catch (Exception e) {
