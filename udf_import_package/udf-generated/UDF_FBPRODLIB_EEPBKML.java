 ...
 * Entity: FBPRODLIB.EEPBKML (DbMoto ReplicationID: 218)
 *
 * Decoded DbMoto field-mapping expressions:
 *   BKCRTD <- MakeDateTime([BKCRTD],[BKCRTT])
 *   BKUPDD <- MakeDateTime([BKUPDD],[BKUPDT])
 *
 ...
 */
public class UDF_FBPRODLIB_EEPBKML {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: IMCO, ESTPRF, ESTYR, ESTNBR, ESTDSH, CYEAR, CNMBR, CDASH, ITEM#, ITMMRK, QTYBAY, MTLTHC, WDTH, NOBRKS, LENGTH, JBFIN, FS#, BAYMRK, PRINT, PRTD, BKCAT, RNDTOP, QTYDSC, BKCRTD, BKUPDD, BKCRTT, BKUPDT, BKCRTU, BKUPDU, BKPGMN
        // Target columns: IMCO, ESTPRF, ESTYR, ESTNBR, ESTDSH, CYEAR, CNMBR, CDASH, ITEM#, ITMMRK, QTYBAY, MTLTHC, WDTH, NOBRKS, LENGTH, JBFIN, FS#, BAYMRK, PRINT, PRTD, BKCAT, RNDTOP, QTYDSC, BKCRTD, BKUPDD, BKCRTU, BKUPDU, BKPGMN

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("BKCRTD", null);
            modified_values.put("BKUPDD", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("BKCRTD", makeDateTime(toLocalDate(newValues.get("BKCRTD")), toLocalTime(newValues.get("BKCRTT"))));
                modified_values.put("BKUPDD", makeDateTime(toLocalDate(newValues.get("BKUPDD")), toLocalTime(newValues.get("BKUPDT"))));
        } catch (Exception e) {
