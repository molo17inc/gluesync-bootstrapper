 ...
 * Entity: KBMEFCOADD.FGDSSTO (DbMoto ReplicationID: 1140)
 *
 * Decoded DbMoto field-mapping expressions:
 *   GDCDTE <- MakeDateTime([GDCDTE])
 *   GDLDTE <- MakeDateTime([GDLDTE])
 *   GDSDTE <- MakeDateTime([GDSDTE])
 *
...
public class UDF_KBMEFCOADD_FGDSSTO {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        ...
        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("GDCDTE", null);
            modified_values.put("GDLDTE", null);
            modified_values.put("GDSDTE", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("GDCDTE", makeDateTime(toLocalDate(newValues.get("GDCDTE"))));
                modified_values.put("GDLDTE", makeDateTime(toLocalDate(newValues.get("GDLDTE"))));
                modified_values.put("GDSDTE", makeDateTime(toLocalDate(newValues.get("GDSDTE"))));
        } catch (Exception e) {
