 ...
 * Entity: ACPRODLIB.ACPAGCAT (DbMoto ReplicationID: 190)
 *
 * Decoded DbMoto field-mapping expressions:
 *   CNCRTDTTM <- MakeDateTime([CNDATCRT], [CNTIMCRT])
 *   CNUPDDTTM <- MakeDateTime([CNDATUPD], [CNTIMUPD])
 *
 ...
 */
public class UDF_ACPRODLIB_ACPAGCAT {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: CUSNBR, ACTTYP, CNCONBR, CNDATCRT, CNDATUPD, CNTIMCRT, CNTIMUPD, CNUSRCRT, CNUSRUPD, CNPGMNAM
        // Target columns: CUSNBR, ACTTYP, CNCONBR, CNCRTDTTM, CNUPDDTTM, CNUSRCRT, CNUSRUPD, CNPGMNAM

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("CNCRTDTTM", null);
            modified_values.put("CNUPDDTTM", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("CNCRTDTTM", makeDateTime(toLocalDate(newValues.get("CNDATCRT")), toLocalTime(newValues.get("CNTIMCRT"))));
                modified_values.put("CNUPDDTTM", makeDateTime(toLocalDate(newValues.get("CNDATUPD")), toLocalTime(newValues.get("CNTIMUPD"))));
        } catch (Exception e) {
