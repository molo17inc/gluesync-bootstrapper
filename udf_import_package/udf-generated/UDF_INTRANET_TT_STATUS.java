 ...
 * Entity: INTRANET.TT_STATUS (DbMoto ReplicationID: 239)
 *
 * Decoded DbMoto field-mapping expressions:
 *   LAST_UPDATED <- MakeDateTime([LAST_UPDATED])
 *
 ...
 */
public class UDF_INTRANET_TT_STATUS {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: STATUS_ID, TASK_ID, STATUS_NUMBER, UPDATE_USER, LAST_UPDATED
        // Target columns: STATUS_ID, TASK_ID, STATUS_NUMBER, UPDATE_USER, LAST_UPDATED

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("LAST_UPDATED", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("LAST_UPDATED", makeDateTime(toLocalDate(newValues.get("LAST_UPDATED"))));
        } catch (Exception e) {
