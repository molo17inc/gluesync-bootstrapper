 ...
 * Entity: ESPRODLIB.FJBMULT (DbMoto ReplicationID: 690)
 *
 * Decoded DbMoto field-mapping expressions:
 *   JBDATETIME <- MakeDateTime([JBDATE],[JBTIME])
 *   TMCRTDATETIME <- MakeDateTime([TMCRTD],[TMCRTT])
 *   TMUPDDATETIME <- MakeDateTime([TMUPDD],[TMUPDT])
 *
 ...
 */
public class UDF_ESPRODLIB_FJBMULT {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: JBCODE, JBRRN, JBMIN$, JBMAX$, JBMULT, JBDATE, JBTIME, JBUSER, TMCO, TMCRTD, TMUPDD, TMCRTT, TMUPDT, TMCRTU, TMUPDU, TMPGMN
        // Target columns: JBCODE, JBRRN, JBMIN$, JBMAX$, JBMULT, JBDATETIME, JBUSER, TMCO, TMCRTDATETIME, TMUPDDATETIME, TMCRTU, TMUPDU, TMPGMN

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("JBDATETIME", null);
            modified_values.put("TMCRTDATETIME", null);
            modified_values.put("TMUPDDATETIME", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("JBDATETIME", makeDateTime(toLocalDate(newValues.get("JBDATE")), toBigDecimal(newValues.get("JBTIME"))));
                modified_values.put("TMCRTDATETIME", makeDateTime(toLocalDate(newValues.get("TMCRTD")), toLocalTime(newValues.get("TMCRTT"))));
                modified_values.put("TMUPDDATETIME", makeDateTime(toLocalDate(newValues.get("TMUPDD")), toLocalTime(newValues.get("TMUPDT"))));
        } catch (Exception e) {
