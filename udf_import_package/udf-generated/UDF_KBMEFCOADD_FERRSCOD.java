 ...
 * Entity: KBMEFCOADD.FERRSCOD (DbMoto ReplicationID: 304)
 *
 * Decoded DbMoto field-mapping expressions:
 *   ESCRTD <- MakeDateTime([ESCRTD])
 *   ESUPDD <- MakeDateTime([ESUPDD])
 *   ESCRTT <- MakeDateTime([ESCRTT])
 *   ESUPDT <- MakeDateTime([ESUPDT])
 *
 ...
 */
public class UDF_KBMEFCOADD_FERRSCOD {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: ESCODE, ESDESC, ESDELETED, ESCRTD, ESUPDD, ESCRTT, ESUPDT, ESCRTU, ESUPDU, ESPGMN, ESBSCD, ESERSC
        // Target columns: RECORDID, ESCODE, ESDESC, ESDELETED, ESCRTD, ESUPDD, ESCRTT, ESUPDT, ESCRTU, ESUPDU, ESPGMN, ESBSCD, ESERSC

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("ESCRTD", null);
            modified_values.put("ESUPDD", null);
            modified_values.put("ESCRTT", null);
            modified_values.put("ESUPDT", null);
            modified_values.put("RECORDID", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("ESCRTD", makeDateTime(toLocalDate(newValues.get("ESCRTD"))));
                modified_values.put("ESUPDD", makeDateTime(toLocalDate(newValues.get("ESUPDD"))));
                modified_values.put("ESCRTT", makeDateTime("MMDDYY", toLocalTime(newValues.get("ESCRTT"))));
                modified_values.put("ESUPDT", makeDateTime("MMDDYY", toLocalTime(newValues.get("ESUPDT"))));
