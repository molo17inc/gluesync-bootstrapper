 ...
 * Entity: KBMEFCOADD.FKSHOP05 (DbMoto ReplicationID: 311)
 *
 * Decoded DbMoto field-mapping expressions:
 *   SHPDAT <- MakeDateTime ("MMDDYY", [SHPDAT] )
 *   #SHDAT <- MakeDateTime("MMDDYY",[#SHDAT])
 *   PRTSHD <- MakeDateTime("MMDDYY",[PRTSHD])
 *
 ...
 */
public class UDF_KBMEFCOADD_FKSHOP05 {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: ITMTYP, SHPN, PRTWC, DESC, IMBUY, SHPDAT, #SHDAT, @CQTY, @SHOH, SHREF, SHTOPN, SHWCTR, PRTSHD
        // Target columns: RECORDID, ITMTYP, SHPN, PRTWC, DESC1, IMBUY, SHPDAT, #SHDAT, CQTY, SHOH, SHREF, SHTOPN, SHWCTR, PRTSHD

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("SHPDAT", null);
            modified_values.put("#SHDAT", null);
            modified_values.put("PRTSHD", null);
            modified_values.put("RECORDID", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("SHPDAT", makeDateTime("MMDDYY", toBigDecimal(newValues.get("SHPDAT"))));
                modified_values.put("#SHDAT", makeDateTime("MMDDYY", toBigDecimal(newValues.get("#SHDAT"))));
                modified_values.put("PRTSHD", makeDateTime("MMDDYY", toBigDecimal(newValues.get("PRTSHD"))));
