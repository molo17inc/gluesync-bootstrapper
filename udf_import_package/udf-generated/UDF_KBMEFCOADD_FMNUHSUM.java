 ...
 * Entity: KBMEFCOADD.FMNUHSUM (DbMoto ReplicationID: 312)
 *
 * Decoded DbMoto field-mapping expressions:
 *   MSFIRUSE <- MakeDateTime([MSFIRUSE])
 *   MSLASUSE <- MakeDateTime([MSLASUSE])
 *
 ...
 */
public class UDF_KBMEFCOADD_FMNUHSUM {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: MSUSERID, MSOPTION, MSCOUNT, MSFIRUSE, MSLASUSE, MSOPTNUM
        // Target columns: MSUSERID, MSOPTION, MSCOUNT, MSFIRUSE, MSLASUSE, MSOPTNUM

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("MSFIRUSE", null);
            modified_values.put("MSLASUSE", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("MSFIRUSE", makeDateTime(toLocalDate(newValues.get("MSFIRUSE"))));
                modified_values.put("MSLASUSE", makeDateTime(toLocalDate(newValues.get("MSLASUSE"))));
        } catch (Exception e) {
