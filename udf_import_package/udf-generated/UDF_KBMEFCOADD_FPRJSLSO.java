 ...
 * Entity: KBMEFCOADD.FPRJSLSO (DbMoto ReplicationID: 318)
 *
 * Decoded DbMoto field-mapping expressions:
 *   PSCRTDTETM <- MakeDateTime([PSCRTDTE],[PSCRTTIM])
 *   PSUPDDTETM <- MakeDateTime([PSUPDDTE],[PSUPDTIM])
 *
 ...
 */
public class UDF_KBMEFCOADD_FPRJSLSO {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: PSPRJNUM, PSSONUM, PSCO, PSTYPE, PSJOBNAM, PSESTPR, PSESTYR, PSESTNBR, PSESTDSH, PSAMT, PSCRTYET, PSCONSTRUC, PSLEED, PSCRTDTE, PSUPDDTE, PSCRTTIM, PSUPDTIM, PSCRTUSR, PSUPDUSR, PSPGMUSD, PSPTXCPT
        // Target columns: PSPRJNUM, PSSONUM, PSCO, PSTYPE, PSJOBNAM, PSESTPR, PSESTYR, PSESTNBR, PSESTDSH, PSAMT, PSCRTYET, PSCONSTRUC, PSLEED, PSCRTDTETM, PSUPDDTETM, PSCRTUSR, PSUPDUSR, PSPGMUSD

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("PSCRTDTETM", null);
            modified_values.put("PSUPDDTETM", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("PSCRTDTETM", makeDateTime(toLocalDate(newValues.get("PSCRTDTE")), toLocalTime(newValues.get("PSCRTTIM"))));
                modified_values.put("PSUPDDTETM", makeDateTime(toLocalDate(newValues.get("PSUPDDTE")), toLocalTime(newValues.get("PSUPDTIM"))));
        } catch (Exception e) {
