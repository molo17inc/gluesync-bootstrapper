 ...
 * Entity: ESPRODLIB.JBPREF (DbMoto ReplicationID: 213)
 *
 * Decoded DbMoto field-mapping expressions:
 *   DATEIS <- MakeDateTime([DATEIS])
 *   JIDATCRT <- MakeDateTime([JIDATCRT],[JITIMCRT])
 *   JIDATUPD <- MakeDateTime([JIDATUPD],[JITIMUPD])
 *
 ...
 */
public class UDF_ESPRODLIB_JBPREF {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: CUSNBR, CUSNAM, CUSTPO, AGTNBR, AGENT, CYEAR, CNMBR, CDASH, ESTPRF, ESTYR, ESTNBR, ESTDSH, JOBNAM, DATEIS, JICONBR, JIDATCRT, JIDATUPD, JITIMCRT, JITIMUPD, JIUSRCRT, JIUSRUPD, JIPGMNAM
        // Target columns: CUSNBR, CUSNAM, CUSTPO, AGTNBR, AGENT, CYEAR, CNMBR, CDASH, ESTPRF, ESTYR, ESTNBR, ESTDSH, JOBNAM, DATEIS, JICONBR, JIDATCRT, JIDATUPD, JIUSRCRT, JIUSRUPD, JIPGMNAM

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("DATEIS", null);
            modified_values.put("JIDATCRT", null);
            modified_values.put("JIDATUPD", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("DATEIS", makeDateTime(toLocalDate(newValues.get("DATEIS"))));
                modified_values.put("JIDATCRT", makeDateTime(toLocalDate(newValues.get("JIDATCRT")), toLocalTime(newValues.get("JITIMCRT"))));
                modified_values.put("JIDATUPD", makeDateTime(toLocalDate(newValues.get("JIDATUPD")), toLocalTime(newValues.get("JITIMUPD"))));
        } catch (Exception e) {
