 ...
 * Entity: ESPRODLIB.ESPUSER (DbMoto ReplicationID: 215)
 *
 * Decoded DbMoto field-mapping expressions:
 *   EGCRTDT <- MakeDateTime([EGDATCRT],[EGTIMCRT])
 *   EGUPDDT <- MakeDateTime([EGDATUPD],[EGTIMUPD])
 *
 ...
 */
public class UDF_ESPRODLIB_ESPUSER {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: USER, UNAME, UGROUP, UGRPL, SYSUSR, EGCONBR, EGDATCRT, EGDATUPD, EGTIMCRT, EGTIMUPD, EGUSRCRT, EGUSRUPD, EGPGMNAM
        // Target columns: USER, UNAME, UGROUP, UGRPL, SYSUSR, EGCONBR, EGCRTDT, EGUPDDT, EGUSRCRT, EGUSRUPD, EGPGMNAM

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("EGCRTDT", null);
            modified_values.put("EGUPDDT", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("EGCRTDT", makeDateTime(toLocalDate(newValues.get("EGDATCRT")), toLocalTime(newValues.get("EGTIMCRT"))));
                modified_values.put("EGUPDDT", makeDateTime(toLocalDate(newValues.get("EGDATUPD")), toLocalTime(newValues.get("EGTIMUPD"))));
        } catch (Exception e) {
