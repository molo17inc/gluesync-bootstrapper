 ...
 * Entity: KBMEFCOADD.FRCVDFECT (DbMoto ReplicationID: 321)
 *
 * Decoded DbMoto field-mapping expressions:
 *   RDCRTDTETM <- MakeDateTime([RDCRTDTE],[RDCRTTME])
 *   RDCHGDTETM <- MakeDateTime([RDCHGDTE],[RDCHGTME])
 *
 ...
 */
public class UDF_KBMEFCOADD_FRCVDFECT {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: RDCO, RDPN, RDCODE, RDQTY, RDNCR#, RDPONO, RDVNDNO, RDLOCCDE, RDLABORCST, RDCMMNT, RDCTRUSR, RDCHGUSR, RDCRTDTE, RDCHGDTE, RDCRTTME, RDCHGTME, RDCOUNT
        // Target columns: RDCO, RDPN, RDCODE, RDQTY, RDNCR#, RDPONO, RDVNDNO, RDLOCCDE, LaborCost, RDCMMNT, RDCTRUSR, RDCHGUSR, RDCRTDTETM, RDCHGDTETM, RDCOUNT

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("RDCRTDTETM", null);
            modified_values.put("RDCHGDTETM", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("RDCRTDTETM", makeDateTime(toLocalDate(newValues.get("RDCRTDTE")), toLocalTime(newValues.get("RDCRTTME"))));
                modified_values.put("RDCHGDTETM", makeDateTime(toLocalDate(newValues.get("RDCHGDTE")), toLocalTime(newValues.get("RDCHGTME"))));
        } catch (Exception e) {
