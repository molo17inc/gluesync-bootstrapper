 ...
 * Entity: KBMEFCOADD.FMREISHD (DbMoto ReplicationID: 315)
 *
 * Decoded DbMoto field-mapping expressions:
 *   DATE <- MakeDateTime([DATE],[TIMERE])
 *   RICRTDTM <- MakeDateTime([RIUPDD],[RIUPDT])
 *   RIUPDDTM <- MakeDateTime([RICRTD],[RICRTT])
 *
...
public class UDF_KBMEFCOADD_FMREISHD {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        ...
        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("DATE", null);
            modified_values.put("RICRTDTM", null);
            modified_values.put("RIUPDDTM", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("DATE", makeDateTime(toLocalDate(newValues.get("DATE")), toBigDecimal(newValues.get("TIMERE"))));
                modified_values.put("RICRTDTM", makeDateTime(toLocalDate(newValues.get("RIUPDD")), toLocalTime(newValues.get("RIUPDT"))));
                modified_values.put("RIUPDDTM", makeDateTime(toLocalDate(newValues.get("RICRTD")), toLocalTime(newValues.get("RICRTT"))));
        } catch (Exception e) {
