 ...
 * Entity: KBMEFCOADD.FESOMM02 (DbMoto ReplicationID: 1223)
 *
 * Decoded DbMoto field-mapping expressions:
 *   PROMISEDAT <- MakeDateTime([PROMISEDAT])
 *   ORDERDATE <- MakeDateTime([ORDERDATE])
 *
 ...
 */
public class UDF_KBMEFCOADD_FESOMM02 {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: SRVGROUP, SRVGRNAME, SRVGREMPNO, SALES_ORD, REPNAME, REP#, PROMISEDAT, ORDERDATE, JOBNAME, INTERCO, GROSSMGN, CUSTNAME, CUST#, COMPANY
        // Target columns: GROSSMGN, PROMISEDAT, ORDERDATE, SRVGREMPNO, SRVGRNAME, SRVGROUP, INTERCO, CUSTNAME, CUST#, REPNAME, REP#, JOBNAME, SALES_ORD, COMPANY

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("PROMISEDAT", null);
            modified_values.put("ORDERDATE", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("PROMISEDAT", makeDateTime(toLocalDate(newValues.get("PROMISEDAT"))));
                modified_values.put("ORDERDATE", makeDateTime(toLocalDate(newValues.get("ORDERDATE"))));
        } catch (Exception e) {
