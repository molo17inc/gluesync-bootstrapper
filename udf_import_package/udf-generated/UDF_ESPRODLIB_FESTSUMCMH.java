 ...
 * Entity: ESPRODLIB.FESTSUMCMH (DbMoto ReplicationID: 686)
 *
 * Decoded DbMoto field-mapping expressions:
 *   QTHCDATETIME <- LowDate(MakeDateTime([QTHCDAT],[QTHCTIM]))
 *
 ...
 */
public class UDF_ESPRODLIB_FESTSUMCMH {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: QTHPRF, QTHYR, QTHNBR, QTHDSH, QTHCDAT, QTHCTIM, QTHUSER
        // Target columns: QTHPRF, QTHYR, QTHNBR, QTHDSH, QTHCDATETIME, QTHUSER

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("QTHCDATETIME", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("QTHCDATETIME", lowDate(makeDateTime(toLocalDate(newValues.get("QTHCDAT")), toLocalTime(newValues.get("QTHCTIM")))));
        } catch (Exception e) {
