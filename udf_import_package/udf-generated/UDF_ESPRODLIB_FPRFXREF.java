 ...
 * Entity: ESPRODLIB.FPRFXREF (DbMoto ReplicationID: 209)
 *
 * Decoded DbMoto field-mapping expressions:
 *   RDCRTD <- MakeDateTime([RDCRTD],[RDCRTT])
 *   RDUPDD <- MakeDateTime([RDUPDD],[RDUPDT])
 *
 ...
 */
public class UDF_ESPRODLIB_FPRFXREF {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: PREFSYSTM, SERIES, PDCATE, RDCRTD, RDUPDD, RDCRTT, RDUPDT, RDCRTU, RDUPDU, RDPGMN
        // Target columns: PREFSYSTM, SERIES, PDCATE, RDCRTD, RDUPDD, RDCRTU, RDUPDU, RDPGMN

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("RDCRTD", null);
            modified_values.put("RDUPDD", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("RDCRTD", makeDateTime(toLocalDate(newValues.get("RDCRTD")), toLocalTime(newValues.get("RDCRTT"))));
                modified_values.put("RDUPDD", makeDateTime(toLocalDate(newValues.get("RDUPDD")), toLocalTime(newValues.get("RDUPDT"))));
        } catch (Exception e) {
