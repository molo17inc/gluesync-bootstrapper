 ...
 * Entity: KBMEFCOADD.FERRCODE (DbMoto ReplicationID: 302)
 *
 * Decoded DbMoto field-mapping expressions:
 *   ERCRTD <- MakeDateTime([ERCRTD],[ERCRTT])
 *   ERUPDD <- MakeDateTime([ERUPDD],[ERUPDT])
 *
 ...
 */
public class UDF_KBMEFCOADD_FERRCODE {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: ERCODE, ERWRKCTR, ERFSPROB, ERDELETED, ERCO, ERCRTD, ERUPDD, ERCRTT, ERUPDT, ERCRTU, ERUPDU, ERPGMN
        // Target columns: ERCODE, ERWRKCTR, ERPRDCAT, ERFSPROB, ERDELETED, ERCO, ERCRTD, ERUPDD, ERCRTU, ERUPDU, ERPGMN

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("ERCRTD", null);
            modified_values.put("ERUPDD", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("ERCRTD", makeDateTime(toLocalDate(newValues.get("ERCRTD")), toLocalTime(newValues.get("ERCRTT"))));
                modified_values.put("ERUPDD", makeDateTime(toLocalDate(newValues.get("ERUPDD")), toLocalTime(newValues.get("ERUPDT"))));
        } catch (Exception e) {
