 ...
 * Entity: KBMEFCOADD.MISSLISTF (DbMoto ReplicationID: 337)
 *
 * Decoded DbMoto field-mapping expressions:
 *   MLDUEDATE <- MakeDateTime([MLDUEDTE])
 *   MLCOMDATE <- MakeDateTime([MLCOMDTE])
 *   MLCRTDTETM <- MakeDateTime([MLCRTDTE],[MLCRTTIM])
 *   MLUPDDTETM <- MakeDateTime([MLUPDDTE],[MLUPDTIM])
 *
 ...
 */
public class UDF_KBMEFCOADD_MISSLISTF {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: MLCONUM, MLWRKCTR, MLFISYR, MLFISWK, MLJOBNUM, MLPART, MLWRKORD, MLMISQTY, MLMISDOL, MLDUEDTE, MLLTEDSC, MLSTATUS, MLCOMDTE, MLCRTDTE, MLUPDDTE, MLCRTTIM, MLUPDTIM, MLCRTUSR, MLUPDUSR, MLPGMUSD
        // Target columns: MLCONUM, MLWRKCTR, MLFISYR, MLFISWK, MLJOBNUM, MLPART, MLWRKORD, MLMISQTY, MLMISDOL, MLDUEDATE, MLLTEDSC, MLSTATUS, MLCOMDATE, MLCRTDTETM, MLUPDDTETM, MLCRTUSR, MLUPDUSR, MLPGMUSD

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("MLDUEDATE", null);
            modified_values.put("MLCOMDATE", null);
            modified_values.put("MLCRTDTETM", null);
            modified_values.put("MLUPDDTETM", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("MLDUEDATE", makeDateTime(toLocalDate(newValues.get("MLDUEDTE"))));
                modified_values.put("MLCOMDATE", makeDateTime(toLocalDate(newValues.get("MLCOMDTE"))));
                modified_values.put("MLCRTDTETM", makeDateTime(toLocalDate(newValues.get("MLCRTDTE")), toLocalTime(newValues.get("MLCRTTIM"))));
                modified_values.put("MLUPDDTETM", makeDateTime(toLocalDate(newValues.get("MLUPDDTE")), toLocalTime(newValues.get("MLUPDTIM"))));
        } catch (Exception e) {
