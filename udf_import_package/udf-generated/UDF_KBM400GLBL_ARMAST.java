 ...
 * Entity: KBM400GLBL.ARMAST (DbMoto ReplicationID: 255)
 *
 * Decoded DbMoto field-mapping expressions:
 *   OPENED <- MakeDateTime("YYYYMMDD",[OPENED])
 *   DTEFOL <- MakeDateTime("YYYYMMDD",[DTEFOL])
 *   INVDTE <- MakeDateTime("YYYYMMDD",[INVDTE])
 *   LSTPDT <- MakeDateTime("YYYYMMDD",[LSTPDT])
 *   DTERVW <- MakeDateTime("YYYYMMDD",[DTERVW])
 *   HIGHDT <- MakeDateTime("YYYYMMDD",[HIGHDT])
 *
 ...
 */
public class UDF_KBM400GLBL_ARMAST {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: ACTIVE, AUDOPR, AUDTIM, BILLTO, CRDLIM, CRDSTS, DAYSPY, FRQSTM, NAMEID, POSTAR, TYPAR, TYPSTM, SHIPTO, SUBSTM, SRVCCD, TERMCD, DISCCD, RESPER, OPENED, DTEFOL, INVDTE, INVAMT, INVNO, LSTPMT, LSTPDT, QTEHLD, OPNORD, CRDHLD, WTSORD, SHPNIV, OPENAR, DSPTAR, DTERVW, HIGHAR, HIGHDT
        // Target columns: ACTIVE, AUDOPR, AUDTIM, BILLTO, CRDLIM, CRDSTS, DAYSPY, FRQSTM, NAMEID, POSTAR, TYPAR, TYPSTM, SHIPTO, SUBSTM, SRVCCD, TERMCD, DISCCD, RESPER, OPENED, DTEFOL, INVDTE, INVAMT, INVNO, LSTPMT, LSTPDT, QTEHLD, OPNORD, CRDHLD, WTSORD, SHPNIV, OPENAR, DSPTAR, DTERVW, HIGHAR, HIGHDT

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("OPENED", null);
            modified_values.put("DTEFOL", null);
            modified_values.put("INVDTE", null);
            modified_values.put("LSTPDT", null);
            modified_values.put("DTERVW", null);
            modified_values.put("HIGHDT", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("OPENED", makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("OPENED"))));
                modified_values.put("DTEFOL", makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("DTEFOL"))));
                modified_values.put("INVDTE", makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("INVDTE"))));
                modified_values.put("LSTPDT", makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("LSTPDT"))));
                modified_values.put("DTERVW", makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("DTERVW"))));
                modified_values.put("HIGHDT", makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("HIGHDT"))));
        } catch (Exception e) {
