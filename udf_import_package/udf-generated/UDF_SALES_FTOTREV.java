 ...
 * Entity: SALES.FTOTREV (DbMoto ReplicationID: 355)
 *
 * Decoded DbMoto field-mapping expressions:
 *   PROMISEDT <- MakeDateTime([PROMISEDT])
 *
 ...
 */
public class UDF_SALES_FTOTREV {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: COMPANY, SLSORDNO, SLSLINENO, LINESTS, DASH, CATEGORY, SERIES, MPSPART, PROMISEDT, QTY, TOTALAMT
        // Target columns: COMPANY, SLSORDNO, SLSLINENO, LINESTS, DASH, CATEGORY, SERIES, MPSPART, PROMISEDT, QTY, TOTALAMT

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("PROMISEDT", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("PROMISEDT", makeDateTime(toLocalDate(newValues.get("PROMISEDT"))));
        } catch (Exception e) {
