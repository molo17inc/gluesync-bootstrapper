 ...
 * Entity: KBMEFCOADD.LATEROUTF (DbMoto ReplicationID: 336)
 *
 * Decoded DbMoto field-mapping expressions:
 *   LRCRTDATETIME <- MakeDateTime([LRCRTDTE], [LRCRTTIM])
 *   LRUPDDATETIME <- MakeDateTime([LRUPDDTE], [LRUPDTIM])
 *
 ...
 */
public class UDF_KBMEFCOADD_LATEROUTF {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: LRCOMNUM, LRWRKCTR, LRLTEDSC, LRRSNCDE, LRCOUNTR, LRSTATUS, LRCRTDTE, LRUPDDTE, LRCRTTIM, LRUPDTIM, LRCRTUSR, LRUPDUSR, LRPGMUSD
        // Target columns: LRCOMNUM, LRWRKCTR, LRLTEDSC, LRRSNCDE, LRCOUNTR, LRSTATUS, LRCRTDATETIME, LRUPDDATETIME, LRCRTUSR, LRUPDUSR, LRPGMUSD

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("LRCRTDATETIME", null);
            modified_values.put("LRUPDDATETIME", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("LRCRTDATETIME", makeDateTime(toLocalDate(newValues.get("LRCRTDTE")), toLocalTime(newValues.get("LRCRTTIM"))));
                modified_values.put("LRUPDDATETIME", makeDateTime(toLocalDate(newValues.get("LRUPDDTE")), toLocalTime(newValues.get("LRUPDTIM"))));
        } catch (Exception e) {
