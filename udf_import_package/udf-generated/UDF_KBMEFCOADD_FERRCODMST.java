 ...
 * Entity: KBMEFCOADD.FERRCODMST (DbMoto ReplicationID: 1097)
 *
 * Decoded DbMoto field-mapping expressions:
 *   ERUPDDATETIME <- MakeDateTime([ERUPDD],[ERUPDT])
 *   ERCRTDATETIME <- MakeDateTime([ERCRTD],[ERCRTT])
 *
 ...
 */
public class UDF_KBMEFCOADD_FERRCODMST {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: ERPGMN, ERUPDU, ERCRTU, ERUPDT, ERCRTT, ERUPDD, ERCRTD, ERDELETED, ERDESC, ERCODE
        // Target columns: ERPGMN, ERUPDU, ERCRTU, ERUPDDATETIME, ERCRTDATETIME, ERDELETED, ERDESC, ERCODE

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("ERUPDDATETIME", null);
            modified_values.put("ERCRTDATETIME", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("ERUPDDATETIME", makeDateTime(toLocalDate(newValues.get("ERUPDD")), toLocalTime(newValues.get("ERUPDT"))));
                modified_values.put("ERCRTDATETIME", makeDateTime(toLocalDate(newValues.get("ERCRTD")), toLocalTime(newValues.get("ERCRTT"))));
        } catch (Exception e) {
