 ...
 * Entity: KBM400D011.MAPEDIT (DbMoto ReplicationID: 242)
 *
 * Decoded DbMoto field-mapping expressions:
 *   DTEAPL <- MakeDateTime("YYYYMMDD",[DTEAPL])
 *   DTEDSC <- MakeDateTime("YYYYMMDD",[DTEDSC])
 *   DTEDUE <- MakeDateTime("YYYYMMDD",[DTEDUE])
 *   DTEEFF <- MakeDateTime("YYYYMMDD",[DTEEFF])
 *   DTETRN <- MakeDateTime("YYYYMMDD",[DTETRN])
 *
 ...
 */
public class UDF_KBM400D011_MAPEDIT {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: ACT, ACTIVE, AMTBSE, AMTDOL, AMTDSC, APDTX, APETBS, APFTBS, APHOLD, APNET, APP, APPCT, APSLTX, AUDOPR, AUDTIM, CD1099, CO, CURRCD, DOCEXT, DOCPO, DOCSRC, DTEAPL, DTEDSC, DTEDUE, DTEEFF, DTETRN, EXRATE, EXSRCE, FAMBSE, FAMDSC, FISCAL, FORAMT, FSCLCC, GLREF, LINED, LOC, MFG, NAMEID, PAYEE, PAYTYP, PERIOD, PPAYMT, PRJECT, PRTPAY, QTR, QTYAMT, REFER, STATUS, STAXCD, SUMREF, TRMDAY, TRMPCT, TYPTRN, VENDNO, YEAR
        // Target columns: RECORDID, ACT, ACTIVE, AMTBSE, AMTDOL, AMTDSC, APDTX, APETBS, APFTBS, APHOLD, APNET, APP, APPCT, APSLTX, AUDOPR, AUDTIM, CD1099, CO, CURRCD, DOCEXT, DOCPO, DOCSRC, DTEAPL, DTEDSC, DTEDUE, DTEEFF, DTETRN, EXRATE, EXSRCE, FAMBSE, FAMDSC, FISCAL, FORAMT, FSCLCC, GLREF, LINED, LOC, MFG, NAMEID, PAYEE, PAYTYP, PERIOD, PPAYMT, PRJECT, PRTPAY, QTR, QTYAMT, REFER, STATUS, STAXCD, SUMREF, TRMDAY, TRMPCT, TYPTRN, VENDNO, YEAR

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("DTEAPL", null);
            modified_values.put("DTEDSC", null);
            modified_values.put("DTEDUE", null);
            modified_values.put("DTEEFF", null);
            modified_values.put("DTETRN", null);
            modified_values.put("RECORDID", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("DTEAPL", makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("DTEAPL"))));
                modified_values.put("DTEDSC", makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("DTEDSC"))));
                modified_values.put("DTEDUE", makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("DTEDUE"))));
                modified_values.put("DTEEFF", makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("DTEEFF"))));
                modified_values.put("DTETRN", makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("DTETRN"))));
