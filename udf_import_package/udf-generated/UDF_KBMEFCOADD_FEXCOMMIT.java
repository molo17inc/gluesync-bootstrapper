 ...
 * Entity: KBMEFCOADD.FEXCOMMIT (DbMoto ReplicationID: 307)
 *
 * Decoded DbMoto field-mapping expressions:
 *   EXDTCOMMIT <- MakeDateTime([EXDTCOMMIT])
 *   EXORCOMMIT <- MakeDateTime([EXORCOMMIT])
 *   EXCRTDTM <- MakeDateTime([EXCRTD],[EXCRTT])
 *   EXUPDUTM <- MakeDateTime([EXUPDU],[EXUPDT])
 *
 ...
 */
public class UDF_KBMEFCOADD_FEXCOMMIT {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: EXCHONO, EXCDASH, EXDTCOMMIT, EXUSRSET, EXUSRCHG, EXORCOMMIT, EXCOMMITO, EXUPDD, EXCRTD, EXCRTT, EXCRTU, EXUPDU, EXUPDT, EXPGMN
        // Target columns: EXCHONO, EXCDASH, EXDTCOMMIT, EXUSRSET, EXUSRCHG, EXORCOMMIT, EXCOMMITO, EXUPDD, EXCRTU, EXPGMN, EXCRTDTM, EXUPDUTM

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("EXDTCOMMIT", null);
            modified_values.put("EXORCOMMIT", null);
            modified_values.put("EXCRTDTM", null);
            modified_values.put("EXUPDUTM", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("EXDTCOMMIT", makeDateTime(toLocalDate(newValues.get("EXDTCOMMIT"))));
                modified_values.put("EXORCOMMIT", makeDateTime(toLocalDate(newValues.get("EXORCOMMIT"))));
                modified_values.put("EXCRTDTM", makeDateTime(toLocalDate(newValues.get("EXCRTD")), toLocalTime(newValues.get("EXCRTT"))));
                modified_values.put("EXUPDUTM", makeDateTime(toLocalDate(newValues.get("EXUPDU")), toLocalTime(newValues.get("EXUPDT"))));
        } catch (Exception e) {
