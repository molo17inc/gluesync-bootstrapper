 ...
 * Entity: ESPRODLIB.FESTSUMCMD (DbMoto ReplicationID: 685)
 *
 * Decoded DbMoto field-mapping expressions:
 *   QTDCDATETIME <- LowDate(MakeDateTime([QTDCDAT],[QTDCTIM]))
 *
 ...
 */
public class UDF_ESPRODLIB_FESTSUMCMD {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: QTDPRF, QTDYR, QTDNBR, QTDDSH, QTDCDAT, QTDCTIM, QTDSEQ, QTDCMNT
        // Target columns: QTDPRF, QTDYR, QTDNBR, QTDDSH, QTDCDATETIME, QTDSEQ, QTDCMNT

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("QTDCDATETIME", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("QTDCDATETIME", lowDate(makeDateTime(toLocalDate(newValues.get("QTDCDAT")), toLocalTime(newValues.get("QTDCTIM")))));
        } catch (Exception e) {
