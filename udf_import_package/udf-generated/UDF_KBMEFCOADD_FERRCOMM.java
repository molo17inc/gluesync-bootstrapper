 ...
 * Entity: KBMEFCOADD.FERRCOMM (DbMoto ReplicationID: 303)
 *
 * Decoded DbMoto field-mapping expressions:
 *   CMCRTDT <- MakeDateTime([CMCRTD], [CMCRTT])
 *   CMUPDDT <- MakeDateTime([CMUPDD], [CMUPDT])
 *
 ...
 */
public class UDF_KBMEFCOADD_FERRCOMM {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: CMCOMP, CYEAR, CNMBRA, CDASH, PRONO, PRBNBR, PRBLNO, PRBLN, CMCRTD, CMCRTT, CMCRTU, CMUPDD, CMUPDT, CMUPDU, CMPGMN
        // Target columns: CMCOMP, CYEAR, CNMBRA, CDASH, PRONO, PRBNBR, PRBLNO, PRBLN, CMCRTDT, CMCRTU, CMUPDDT, CMUPDU, CMPGMN

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("CMCRTDT", null);
            modified_values.put("CMUPDDT", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("CMCRTDT", makeDateTime(toLocalDate(newValues.get("CMCRTD")), toLocalTime(newValues.get("CMCRTT"))));
                modified_values.put("CMUPDDT", makeDateTime(toLocalDate(newValues.get("CMUPDD")), toLocalTime(newValues.get("CMUPDT"))));
        } catch (Exception e) {
