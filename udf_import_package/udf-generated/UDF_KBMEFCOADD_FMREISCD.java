 ...
 * Entity: KBMEFCOADD.FMREISCD (DbMoto ReplicationID: 313)
 *
 * Decoded DbMoto field-mapping expressions:
 *   RECRTDTM <- MakeDateTime([RECRTD],[RECRTT])
 *   REUPDDTM <- MakeDateTime([REUPDD],[REUPDT])
 *
 ...
 */
public class UDF_KBMEFCOADD_FMREISCD {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: REICOD, REIDSC, RECO, RECRTD, REUPDD, RECRTT, REUPDT, RECRTU, REUPDU, REPGMN
        // Target columns: REICOD, REIDSC, RECO, RECRTDTM, REUPDDTM, RECRTU, REUPDU, REPGMN

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("RECRTDTM", null);
            modified_values.put("REUPDDTM", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("RECRTDTM", makeDateTime(toLocalDate(newValues.get("RECRTD")), toLocalTime(newValues.get("RECRTT"))));
                modified_values.put("REUPDDTM", makeDateTime(toLocalDate(newValues.get("REUPDD")), toLocalTime(newValues.get("REUPDT"))));
        } catch (Exception e) {
