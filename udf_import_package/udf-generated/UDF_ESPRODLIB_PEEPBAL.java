 ...
 * Entity: ESPRODLIB.PEEPBAL (DbMoto ReplicationID: 1065)
 *
 * Decoded DbMoto field-mapping expressions:
 *   BCUPDDT <- MakeDateTime([BCUPDD], [BCUPDT])
 *   BCCRTDT <- MakeDateTime([BCCRTD], [BCCRTT])
 *
 ...
 */
public class UDF_ESPRODLIB_PEEPBAL {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: BSERIES, QTYSH, BALLNG, VNTWGT, BALNBR, BALLMT, DATSYS, TIMSYS, NEWCST, BALCST, BCCO, BCCRTD, BCUPDD, BCCRTT, BCUPDT, BCCRTU, BCUPDU, BCPGMN
        // Target columns: BCPGMN, BCUPDU, BCCRTU, BCUPDDT, BCCRTDT, BCCO, BALCST, TIMSYS, DATSYS, BALLMT, VNTWGT, BALLNG, QTYSH, NEWCST, BALNBR, BSERIES

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("BCUPDDT", null);
            modified_values.put("BCCRTDT", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("BCUPDDT", makeDateTime(toLocalDate(newValues.get("BCUPDD")), toLocalTime(newValues.get("BCUPDT"))));
                modified_values.put("BCCRTDT", makeDateTime(toLocalDate(newValues.get("BCCRTD")), toLocalTime(newValues.get("BCCRTT"))));
        } catch (Exception e) {
