 ...
 * Entity: ESPRODLIB.FPARTFCT (DbMoto ReplicationID: 208)
 *
 * Decoded DbMoto field-mapping expressions:
 *   FctDat <- MakeDateTime([FCTDAT])
 *   FcteDat <- MakeDateTime([FCTEDAT])
 *   PfCrtDT <- MakeDateTime([PFCRTD], [PFCRTT])
 *
 ...
 */
public class UDF_ESPRODLIB_FPARTFCT {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: PFPART, PFCLS, WGTFCT, CSTFCT, ENGCSTSAV, IMWHTNEW, IMLLTLNEW, PARTOLD, IMWHTOLD, IMLLTLOLD, FCTDAT, FCTEDAT, ACTCOD, PFDESC, PFCOMM, PFCOMM1, PFCOMM2, PFCRTD, PFCRTT, PFCRTU, PFPGMN
        // Target columns: PfPart, PfCls, WgtFct, CstFct, EngCstSav, ImWhtNew, ImLltlNew, PartOld, ImWhtOld, ImLltlOld, FctDat, FcteDat, ActCod, PfDesc, PfComm, PfComm1, PfComm2, PfCrtDT, PfCrtU, PfPgmn

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("FctDat", null);
            modified_values.put("FcteDat", null);
            modified_values.put("PfCrtDT", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("FctDat", makeDateTime(toLocalDate(newValues.get("FCTDAT"))));
                modified_values.put("FcteDat", makeDateTime(toLocalDate(newValues.get("FCTEDAT"))));
                modified_values.put("PfCrtDT", makeDateTime(toLocalDate(newValues.get("PFCRTD")), toLocalTime(newValues.get("PFCRTT"))));
        } catch (Exception e) {
