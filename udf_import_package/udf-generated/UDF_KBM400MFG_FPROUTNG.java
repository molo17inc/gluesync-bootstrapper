 ...
 * Entity: KBM400MFG.FPROUTNG (DbMoto ReplicationID: 279)
 *
 * Decoded DbMoto field-mapping expressions:
 *   RTEFDATE <- LowDate(MakeDateTime([RTEFMO],[RTEFDA],[RTEFYR],[RTEFCT]))
 *   RTEXDATE <- LowDate(MakeDateTime([RTEXMO],[RTEXDA],[RTEXYR],[RTEXCT]))
 *   RTUDTE <- MakeDateTime(MakeDateTime("YYYYMMDD",[RTUDTE]),[RTUTIM])
 *
...
public class UDF_KBM400MFG_FPROUTNG {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        ...
        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("RTEFDATE", null);
            modified_values.put("RTEXDATE", null);
            modified_values.put("RTUDTE", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("RTEFDATE", lowDate(makeDateTime(toBigDecimal(newValues.get("RTEFMO")), toBigDecimal(newValues.get("RTEFDA")), toBigDecimal(newValues.get("RTEFYR")), toBigDecimal(newValues.get("RTEFCT")))));
                modified_values.put("RTEXDATE", lowDate(makeDateTime(toBigDecimal(newValues.get("RTEXMO")), toBigDecimal(newValues.get("RTEXDA")), toBigDecimal(newValues.get("RTEXYR")), toBigDecimal(newValues.get("RTEXCT")))));
                modified_values.put("RTUDTE", makeDateTime((LocalDate)makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("RTUDTE"))), toBigDecimal(newValues.get("RTUTIM"))));
        } catch (Exception e) {
