 ...
 * Entity: KBM400MFG.FHINVDET (DbMoto ReplicationID: 261)
 *
 * Decoded DbMoto field-mapping expressions:
 *   INDDATE <- MakeDateTime([INDMO], [INDDA], [INDYR], [INDCT])
 *   INDUDE <- MakeDateTime( MakeDateTime ("YYYYMMDD",[INDUDE]) , [INDUTI])
 *
...
public class UDF_KBM400MFG_FHINVDET {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        ...
        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("INDDATE", null);
            modified_values.put("INDUDE", null);
            modified_values.put("RECORDID", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("INDDATE", makeDateTime(toBigDecimal(newValues.get("INDMO")), toBigDecimal(newValues.get("INDDA")), toBigDecimal(newValues.get("INDYR")), toBigDecimal(newValues.get("INDCT"))));
                modified_values.put("INDUDE", makeDateTime((LocalDate)makeDateTime("YYYYMMDD", toBigDecimal(newValues.get("INDUDE"))), toBigDecimal(newValues.get("INDUTI"))));
