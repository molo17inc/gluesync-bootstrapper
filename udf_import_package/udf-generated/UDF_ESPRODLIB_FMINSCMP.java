 ...
 * Entity: ESPRODLIB.FMINSCMP (DbMoto ReplicationID: 998)
 *
 * Decoded DbMoto field-mapping expressions:
 *   CPDATCRT <- MakeDateTime([CPDATCRT], [CPTIMCRT])
 *   CPDATUPD <- MakeDateTime([CPDATUPD], [CPTIMUPD])
 *
 ...
 */
public class UDF_ESPRODLIB_FMINSCMP {
    ...
    public Pair<MappingFunctionOperation, Map<String, Object>> onChange(Map<String, Object> newValues, Map<String, Object> oldValues, MappingFunctionOperation operation, Logger logger) {
        // Source columns: CMPSTE, DESC, EXTDIE, INTDIE, INSNUM, QTYINS, CPCONBR, MISCPART, MISCQTY, INSNUM2, QTYINS2, CPDATCRT, CPDATUPD, CPTIMCRT, CPTIMUPD, CPUSRCRT, CPUSRUPD, CPPGMNAM
        // Target columns: CMPSTE, DESC, EXTDIE, INTDIE, INSNUM, QTYINS, CPCONBR, MISCPART, MISCQTY, INSNUM2, QTYINS2, CPDATCRT, CPDATUPD, CPUSRCRT, CPUSRUPD, CPPGMNAM

        Map<String, Object> modified_values = newValues;

        if (operation == MappingFunctionOperation.Delete) {
            modified_values.put("CPDATCRT", null);
            modified_values.put("CPDATUPD", null);
            return new Pair<>(operation, modified_values);
        }

        try {
                modified_values.put("CPDATCRT", makeDateTime(toLocalDate(newValues.get("CPDATCRT")), toLocalTime(newValues.get("CPTIMCRT"))));
                modified_values.put("CPDATUPD", makeDateTime(toLocalDate(newValues.get("CPDATUPD")), toLocalTime(newValues.get("CPTIMUPD"))));
        } catch (Exception e) {
